...
function App() {
  const isLimelightActive = useLimelightStore(
    (state) => state.isLimelightActive,
  );
  const isChatbotOpen = useLimelightStore((state) => state.isChatbotOpen);
  const isProjectDetailsOpen = useLimelightStore(
    (state) => state.isProjectDetailsOpen,
  );
  const deactivateLimelight = useLimelightStore(
    (state) => state.deactivateLimelight,
  );
  ...
  useEffect(() => {
    ...
    console.log("[Limelight Debug] Mount checking URL:", {
    ...
    if (prevIsLimelightActive.current && !isLimelightActive) {
      ...
      if (lastSegment && urlToLimelightId[lastSegment]) {
        ...
        newPathname =
          "/" +
          pathSegments.join("/") +
          (pathSegments.length > 0 || window.location.pathname.endsWith("/")
            ? "/"
            : "");
        changed = true;
      }
      ...
      if (changed) {
        console.log(
          "[Limelight Debug] Cleaning up URL to:",
